...
pnpm eval:answer -- --refusal # 生成层评测：只跑 9 道拒答题（约 $1.5）
pnpm eval:answer              # 生成层评测：全部 28 题跑 1 轮（约 $5）
pnpm eval:answer -- --repeat 3  # 跑 3 轮报均值±方差（一次满分不叫基线）
...
- [x] Day 4 加固：4 道「半知半解」型对抗题 + 重复采样与逐题一致性
- [ ] Day 4 遗留：向量库选型（暴力 vs HNSW）
